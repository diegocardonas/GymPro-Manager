...
const RoutineTemplates: React.FC = () => {
    ...
    const handleDelete = (templateId: string) => {
        if (window.confirm('Are you sure you want to delete this routine template? This action cannot be undone.')) {
    ...
    return (
        <div className="w-full space-y-8">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Routine Templates</h2>
...
const RoutineTemplateModal: React.FC<{ template: PreEstablishedRoutine | null, onSave: (template: PreEstablishedRoutine) => void, onClose: () => void }> = ({ template, onSave, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-scale-in">
                <h2 className="text-2xl font-bold p-6 border-b border-gray-200 dark:border-gray-700">{template ? 'Edit' : 'Create'} Routine Template</h2>
                <div className="p-6 space-y-4 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-gray-600 dark:text-gray-400">Template Name</label>
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div className="flex-1 p-6 overflow-y-auto">
                         <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4">{activeDay} Routine</h3>
                         <div className="space-y-3">
                            {activeDayRoutine && activeDayRoutine.exercises.length > 0 && (
                                <div className="flex items-center gap-2">
                                    <div className="flex-grow text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Exercise</div>
                                    <div className="w-28 flex-shrink-0 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Sets</div>
                                    <div className="w-32 flex-shrink-0 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Reps</div>
                            ...
                            {activeDayRoutine?.exercises.map((ex, exIndex) => (
                                <div key={exIndex} className="flex items-center gap-2">
                                    <select value={ex.name} onChange={(e) => handleExerciseChange(exIndex, 'name', e.target.value)} className="flex-grow input-style">
                                    ...
                                    <button type="button" onClick={() => handleRemoveExercise(exIndex)} className="p-2 flex-shrink-0 text-gray-400 hover:text-red-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"><TrashIcon className="h-5 w-5" /></button>
                                </div>
                            ))}
                         </div>
                         {(!activeDayRoutine || activeDayRoutine.exercises.length === 0) && <div className="text-center py-10 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg"><p className="font-semibold text-gray-500 dark:text-gray-400">Rest Day</p></div>}
                        <div className="mt-4">
                ...
                <div className="flex justify-end space-x-4 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 sticky bottom-0">
                    <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 rounded-lg font-semibold transition-colors text-gray-800 dark:text-gray-200">Cancel</button>
                    <button type="submit" className="px-4 py-2 bg-primary hover:bg-primary/90 rounded-lg font-semibold transition-colors text-primary-foreground">Save Template</button>
                </div>
