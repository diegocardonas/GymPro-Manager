...
const ClientSidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen, onClose }) => {
    const navItems = [
        ...
        { id: 'workout-log', label: 'Registrar Entrenamiento', icon: PencilIcon },
        { id: 'progress', label: 'Mi Progreso', icon: ChartBarIcon },
        { id: 'nutrition-log', label: 'Registro Nutricional', icon: AppleIcon },
        ...
        { id: 'membership-card', label: 'Tarjeta de Miembro', icon: IdentificationIcon },
    ...
    return (
        <div className={`w-64 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm border-r border-black/10 dark:border-white/10 p-4 flex flex-col fixed h-full z-30 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex items-center gap-2 mb-10 px-2 pt-2">
                ...
                <span className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-blue-600 dark:from-teal-300 dark:to-blue-400">
                    GymPro
                </span>
            </div>
