...
const TrainerRoutineTemplates: React.FC = () => {
    const { currentUser, preEstablishedRoutines, addRoutineTemplate, updateRoutineTemplate, deleteRoutineTemplate } = useContext(AuthContext);
    ...
    const trainerTemplates = useMemo(() => {
        return preEstablishedRoutines.filter(r => r.trainerId === currentUser?.id);
    }, [preEstablishedRoutines, currentUser]);
    ...
    const handleSave = (template: PreEstablishedRoutine) => {
        ...
        if (template.id) {
            ...
            addRoutineTemplate({ ...template, id: `rt-trainer-${Date.now()}`, trainerId: currentUser.id });
        }
    ...
    return (
        <div className="w-full space-y-8">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Mis Plantillas de Rutina</h2>
            ...
            {trainerTemplates.length > 0 ? (
                ...
                <div className="text-center py-12 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
                    <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300">No se encontraron plantillas de rutina</h3>
                    <p className="mt-2 text-gray-500 dark:text-gray-400">Crea tu primera plantilla para reutilizarla con tus clientes.</p>
                </div>
            )}
    ...
    return (
        <div className="bg-white dark:bg-gray-800/50 rounded-xl ring-1 ring-black/5 dark:ring-white/10 shadow-lg flex flex-col">
            <div className="p-6 flex-grow">
                ...
                <span className="px-3 py-1 text-xs font-semibold rounded-full bg-primary/10 text-primary">{workoutDays} día(s) de entrenamiento</span>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 space-y-4 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-gray-600 dark:text-gray-400">Nombre de la Plantilla</label>
                     ...
                     <div>
                        <label htmlFor="description" className="block text-sm font-medium text-gray-600 dark:text-gray-400">Descripción</label>
                ...
                <div className="flex justify-end space-x-4 p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 sticky bottom-0">
                    ...
                    <button type="submit" className="px-4 py-2 bg-primary hover:bg-primary/90 rounded-lg font-semibold transition-colors text-primary-foreground">Guardar Plantilla</button>
                </div>
...
export default TrainerRoutineTemplates;
